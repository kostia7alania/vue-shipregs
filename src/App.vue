...
export default {
    ...
    axios
      ...
      .then(e => {
        ...
        if (d instanceof Array && d.length > 0) {
          ///[ {value: null,text:'все'}, {value:"RUAZO",text:"Азов"},..]
          this.ports = d;
      ...
      .catch(err => {
        ...
        this.$toast.warning(
          "Произошла ошибка при получении списка портов",
          "Сетевая ошибка",
  ...
  methods: {
    ...
    graf_refresh({ data, action = "getUsers" }) {
      //actions => 'getInport'(уже нету!); 'getUsers';
      ...
      if (action == "toSend") this.transferRows(data, "toSend");
        ...
        axios
          ...
          .then(res => {
            let data = res.data;
            if (typeof data == "string" && data.match("SQL") != null)
              throw "MS SQL ERROR!";
            if (action === "toExport") {
